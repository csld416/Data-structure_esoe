...
template <typename T>
DList<T>::DList() {
    head = new DListNode<T>(T(), nullptr, nullptr);
    head->next = head;
    head->prev = head;
    size = 0;
}
...
void DList<T>::insertFront(const T& item) {
    DListNode<T>* newNode = this->newNode(item, head, head->next);
...
void DList<T>::insertBack(const T& item) {
    DListNode<T>* newNode = this->newNode(item, head->prev, head);
...
DListNode<T>* DList<T>::prev(DListNode<T>* node) {
    if (node == nullptr || node->prev == head) {
        return nullptr;
    }
    return node->prev;
}
...
void DList<T>::insertAfter(const T& item, DListNode<T>* node) {
    ...
    DListNode<T>* newNode = this->newNode(item, node, node->next);
...
void DList<T>::insertBefore(const T& item, DListNode<T>* node) {
    ...
    DListNode<T>* newNode = this->newNode(item, node->prev, node);
